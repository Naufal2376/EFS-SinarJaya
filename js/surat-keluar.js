...
$(document).ready(function () {
        ...
        render: function (data, type, row) {
          return `
            <div class="flex space-x-2 justify-center">
              ...
              ${
                row.PDF
                  ? `<button onclick=\"printPDF('surat-keluar','${row.PDF}')\" class=\"px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700\" title=\"Print / Lihat PDF\"><i class='fas fa-print'></i></button>`
                  : ""
              }
            </div>`
        },
  ...
  window.editData = function (id) {
    window.location.href = `form-surat-keluar.html?id=${id}`
  }
  ...
  window.exportData = function () {
    ...
    a.href = url
    a.download = "arsip_surat_keluar_reforma_indah.json"
    a.click()
  ...
  window.clearAllData = function () {
    Swal.fire({
      ...
      text: "Tindakan ini akan menghapus semua arsip surat keluar!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#d33",
      ...
      if (result1.isConfirmed) {
        ...
        Swal.fire({
          ...
          if (result2.isConfirmed) {
            ...
            table.column(6).search(perihal, true, false)
            table.draw()
          }
        })
      }
